...
**輸出範例**（B 細胞淋巴瘤 v3.2025）：
- 34 個參考檔案，涵蓋 15+ 種疾病亞型
- 8,592 行結構化臨床內容
- 3,479 個頁碼引用（`[p.XX]`），可追溯至原始 PDF
- 漸進式揭露：SKILL.md（147 行）→ 疾病演算法 → 實證摘要
...
nccn-b-cell-lymphomas/            # 生成的技能套件（範例輸出）
├── SKILL.md                      # 導覽入口，具漸進式揭露
└── references/                   # 34 個疾病特定 Markdown 檔案
```
